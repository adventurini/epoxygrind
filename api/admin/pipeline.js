 ...
 *
 * GET ?contractorId=N -> { stage, answered, notes: [{id, note, createdAt}] }
 * PATCH { contractorId, stage?, answered? } -> upserts the pipeline row
 * POST { contractorId, note } -> appends a note
 */
  ...
  if (req.method === 'GET') {
    ...
    try {
      const [{ data: pipeline }, { data: notes }] = await Promise.all([
        ...
        supabase.from('contractor_notes').select('id, note, created_at').eq('contractor_id', contractorId).order('created_at', { ascending: false }),
      ]);
  ...
  if (req.method === 'POST') {
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({ contractor_id: contractorId, note: note.slice(0, 5000) })
        .select('id, note, created_at')
        .single();
      if (error) throw error;
      return res.status(200).json({ id: data.id, note: data.note, createdAt: data.created_at });
    } catch (err) {
      console.error(err);
