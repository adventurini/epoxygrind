...
export async function generateAllEstimatePreviews(supabase, userId, estimateId, row) {
  ...
  // Also save this as a permanent, never-overwritten history entry (not
  // just the always-latest 'original' pointer) — the site owner asked for
  // every generated version to be saved and shown, including the very
  // first one, not only later regenerations (which already accumulate
  // history client-side, see app/estimate/estimate.js's accumulatePreviews).
  // persistEstimateImages seeds stored.previewPaths from row.payload's
  // existing previewPaths first, so any prior history from an earlier call
  // is preserved automatically, not overwritten by adding this one.
  const previews = [
    { id: original.id, label: original.label, image: original.image },
    { id: `hist-${Date.now()}`, label: original.label, image: original.image, createdAt: new Date().toISOString() },
  ];
