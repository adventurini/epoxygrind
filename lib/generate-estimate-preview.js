...
const ANGLE_IDS = PREVIEW_ANGLES.filter((item) => item.mode === 'angle').map((item) => item.id);
...
/**
 * Generate all four floor preview angles server-side and persist to storage.
 * Original is generated first so door/left/right use the edited hero floor.
 */
export async function generateAllEstimatePreviews(supabase, userId, estimateId, row) {
  ...
  try {
    ...
    previews.push({ id: original.id, label: original.label, image: original.image });
    if (original.heroImage) {
      ctx.heroImage = original.heroImage;
    }
  } catch (err) {
    console.error('Preview original failed:', err.message);
  }
  ...
  for (const angleId of ANGLE_IDS) {
    try {
      ...
      previews.push({ id: result.id, label: result.label, image: result.image });
    } catch (err) {
