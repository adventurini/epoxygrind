...
export function productLinkHtml(productId, label) {
  ...
  return `<a class="product-link" href="/go/${encodeURIComponent(productId)}" rel="${resolved.rel}" target="_blank" data-merchant="${escapeHtml(resolved.product.merchant)}">${text}</a>`;
}
 ...
 *
 * Shows the product photo and price when the registry has them. image_url
 * is either a real photo fetched from the live merchant page, or (rare,
 * flagged via image_generated — see product-registry.js header comment)
 * an AI-generated brand-neutral placeholder used only when a real photo
 * couldn't be recovered; price_text is only ever a real observed price,
 * never fabricated. Cards fall back to a text-only layout when neither is
 * available rather than showing a broken image or a guessed price.
 */
...
export function comparisonTableHtml(data) {
  const rows = data.rows
    .map((row) => {
      ...
      return `<tr><td class="name-cell"><a class="product-link" href="/go/${encodeURIComponent(row.productId)}" rel="${resolved.rel}" target="_blank" data-merchant="${escapeHtml(resolved.product.merchant)}">${escapeHtml(resolved.product.display_name)}</a></td>${cells}</tr>`;
    })
    .join('');
...
export function proCtaHtml() {
  return `<div class="pro-cta">
    ...
    <a class="btn btn-p" href="/estimator/">Get my instant estimate →</a>
  </div>`;
}

/** Contractor-facing CTA for /learn/ articles — same visual component as
 * proCtaHtml() but pointed at the contractor funnel, not the homeowner
 * estimator. */
export function contractorCtaHtml() {
  return `<div class="pro-cta">
    <div class="pro-cta-text">
      <h3>Want us to just fix this for you?</h3>
      <p>Free audit shows you exactly what's costing you jobs on your own site — no obligation.</p>
    </div>
    <a class="btn btn-p" href="/audit/">Get my free audit →</a>
  </div>`;
}
      ...
      const rows = data.items
        ...
        .map((item) => {
          ...
          return `<div class="shopping-list-row">
            <a class="product-link" href="/go/${encodeURIComponent(item.productId)}" rel="${resolved.rel}" target="_blank" data-merchant="${escapeHtml(resolved.product.merchant)}">${escapeHtml(resolved.product.display_name)}</a>
