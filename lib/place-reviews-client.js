...
/**
 * Hero figcaption + gallery grid — populated from `data.photo_attributions`
 * (author name/link only, never the photo bytes or API key; see
 * sanitizeForClient in supabase/functions/place-reviews). Actual photo
 * bytes are served by /api/places-photo?place_id=..&index=N, a
 * key-protecting proxy, never a Google URL directly. Renders lazily so an
 * unclaimed listing with zero real photos never shows a wall of repeated
 * placeholder images.
 */
function renderPhotos(placeId, data) {
  const attributions = (data?.photo_attributions || []).filter(Boolean);

  const caption = document.querySelector('[data-photo-caption]');
  if (caption && attributions[0]) {
    caption.textContent = `Photo: ${attributions[0].display_name} via Google`;
    caption.hidden = false;
  }

  const gallery = document.querySelector('[data-photo-gallery]');
  if (gallery && attributions.length > 1) {
    const items = attributions.slice(1, 8).map((_, i) => {
      const index = i + 1;
      const src = `/api/places-photo?place_id=${encodeURIComponent(placeId)}&index=${index}`;
      return `<a href="${src}" target="_blank" rel="noopener"><img src="${src}" alt="${escapeHtml(gallery.dataset.contractorName || '')} — customer photo" loading="lazy"></a>`;
    });
    gallery.insertAdjacentHTML('beforebegin', '<h2>Photos</h2>');
    gallery.innerHTML = items.join('');
    gallery.insertAdjacentHTML('afterend', '<p class="tiny muted">Photos from Google.</p>');
...
export async function initPlaceReviews(config) {
  ...
  try {
    ...
    if (result.status !== 'pending') {
      ...
      renderPhotos(placeId, result.data);
      return;
    }
    ...
    for (let attempt = 0; attempt < MAX_POLLS; attempt++) {
      ...
      try {
        ...
        if (retry.status !== 'pending') {
          ...
          renderPhotos(placeId, retry.data);
          return;
        }
      } catch {
        break;
