  ...
  if (Array.isArray(payload.previews) && payload.previews.length) {
    ...
    for (const preview of payload.previews) {
      ...
      if (preview.image?.startsWith('http')) {
        ...
        upsertPreviewPath(stored, { id: preview.id, label: preview.label, path, createdAt: preview.createdAt || new Date().toISOString() });
        continue;
      }
  ...
  if (payload.previewPaths?.length) {
    out.previews = await Promise.all(
      payload.previewPaths.map(async (preview) => ({
        ...
        // Round-tripped back to the client so a later PATCH can reference
        // an already-uploaded preview by storagePath (see
        // persistEstimateImages above) instead of re-uploading it from its
        // (possibly since-expired) fal.media URL — see the "generated
        // floor history" gallery in calculator/estimate-view.js.
