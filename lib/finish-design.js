...
/**
 * Real, industry-standard acid-stain/water-based-stain color names (the
 * naming convention used across concrete-stain manufacturers like Kemiko,
 * Scofield, and Surecrete — not epoxy base coats, which is why this is a
 * separate palette from BASE_COLORS above rather than reusing it). Hexes
 * are representative approximations of each color's typical mottled/
 * variegated look, same convention as BASE_COLORS/FLAKE_COLORS.
 */
export const CONCRETE_COLORS = [
  { id: 'natural-gray', label: 'Natural gray', hex: '#9C9890', prompt: 'natural unstained polished concrete gray' },
  { id: 'charcoal-stain', label: 'Charcoal', hex: '#3C3C3C', prompt: 'charcoal acid-stained concrete' },
  { id: 'coffee-brown', label: 'Coffee brown', hex: '#4A3728', prompt: 'coffee brown acid-stained concrete' },
  { id: 'walnut', label: 'Walnut', hex: '#5C4033', prompt: 'walnut acid-stained concrete' },
  { id: 'sahara-tan', label: 'Sahara tan', hex: '#C2A878', prompt: 'sahara tan acid-stained concrete' },
  { id: 'terra-cotta', label: 'Terra cotta', hex: '#B5651D', prompt: 'terra cotta acid-stained concrete' },
  { id: 'english-red', label: 'English red', hex: '#8B3A2E', prompt: 'english red acid-stained concrete' },
  { id: 'slate-gray', label: 'Slate gray', hex: '#6E7B7F', prompt: 'slate gray acid-stained concrete' },
];
...
export const PATTERNS = {
  ...
  // Not an epoxy/polyaspartic coating system at all — mechanically ground,
  // stained, or decoratively overlaid concrete, sealed. Priced off its own
  // lower baseline in lib/pricing.js's FINISH_OPTIONS.concrete rather than
  // an epoxy rate; patterns here upcharge over that baseline the same way
  // flake's patterns do (e.g. double-broadcast) rather than introducing a
  // second pricing mechanism.
  concrete: [
    {
      id: 'polished',
      label: 'Polished (no stain)',
      description: 'Mechanically ground and polished to a satin-to-high-gloss sheen',
      addLow: 0,
      addHigh: 0,
      prompt: 'smooth mechanically polished concrete floor with a satin-to-high-gloss sheen and a natural stone-like appearance, no added color',
    },
    {
      id: 'stained',
      label: 'Stained & sealed',
      description: 'Acid or water-based stain with a sealed satin finish',
      addLow: 1,
      addHigh: 2,
      prompt: 'acid-stained concrete floor with rich mottled, variegated color and a sealed satin finish',
    },
    {
      id: 'overlay-smooth',
      label: 'Smooth overlay',
      description: 'Thin decorative micro-topping resurfacing over the existing slab',
      addLow: 2,
      addHigh: 4,
      prompt: 'smooth decorative concrete micro-topping overlay with a clean, uniform resurfaced finish',
    },
    {
      id: 'stamped',
      label: 'Stamped overlay',
      description: 'Textured decorative overlay stamped with a stone or slate pattern',
      addLow: 6,
      addHigh: 10,
      prompt: 'decorative stamped concrete overlay with a textured ashlar-slate stamped pattern and tooled joint lines, sealed',
    },
  ],
};
...
export function resolveDesign(input) {
  const finish = ['solid', 'flake', 'metallic', 'concrete'].includes(input.finish) ? input.finish : 'flake';
  // Concrete uses its own stain-color palette (CONCRETE_COLORS), not epoxy
  // base coats — a "charcoal epoxy base coat" isn't a real product a
  // concrete contractor stains with, so BASE_COLORS' ids/hexes don't apply.
  const base = finish === 'concrete'
    ? findColor(input.baseColor || 'natural-gray', CONCRETE_COLORS)
    : findColor(input.baseColor || 'charcoal', BASE_COLORS);
