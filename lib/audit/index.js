...
import { sweepStaleChromiumTempDirs } from './browser-launch.js';
...
/** Live photo count from places_cache (the same 30-day-TTL cache
 * /api/places-photo reads) — replaces the old getContractorHero() lookup
 * against a locally rehosted copy, which was a Places API ToS violation. */
async function getGooglePhotoCount(placeId) {
  if (!placeId || !isSupabaseConfigured()) return 0;
  const supabase = getSupabase();
  const { data } = await supabase.from('places_cache').select('data').eq('place_id', placeId).maybeSingle();
...
export async function runAudit(contractor, localMedianReviewCount) {
  ...
  // A warm Vercel container reused across many invocations can accumulate
  // leftover Chromium temp profile dirs from invocations that got hard-
  // killed (maxDuration, or the crashes below) before their own cleanup
  // ran — confirmed real: one container's /tmp filled completely
  // (FILE_ERROR_NO_SPACE) and every subsequent request failed regardless of
  // target site. Sweep once here, before any of THIS invocation's own
  // browsers exist, rather than inside launchChromium() itself (which would
  // race the two concurrent same-wave launches below).
  if (process.env.VERCEL) await sweepStaleChromiumTempDirs();
  ...
  if (process.env.VERCEL) {
    ...
    // Sweep again between waves, not just once at the top of this function —
    // a hard-killed browser from THIS invocation's own first wave (a real,
    // observed failure mode under back-to-back requests: a crashed launch
    // leaves its /tmp profile dir behind with nothing to clean it up until
    // some LATER invocation's top-of-function sweep runs) would otherwise
    // sit there consuming space for the rest of this invocation's second
    // wave too, compounding the same leak within a single request instead
    // of just across requests.
    await sweepStaleChromiumTempDirs();
  ...
  const photoCount = await getGooglePhotoCount(contractor.place_id);
  ...
  const categoryResults = {
    ...
    localPresence: scoreLocalPresence(contractor, photoCount, localMedianReviewCount, crawl),
