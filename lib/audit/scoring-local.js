 ...
 * network calls, everything already exists from the Places scrape
 * (batch-fetch-places.js) and this session's contractor-images pipeline.
 ...
 * @param {object|null} hero getContractorHero() result — real GBP photos on file
 ...
 */
export function scoreLocalPresence(contractor, hero, localMedianReviewCount, crawl) {
  let score = 0;
  const checks = [];

  const hasPhotos = Boolean(hero);
  if (hasPhotos) score += 25;
  checks.push({
    label: 'Google Business Profile photos',
    value: hasPhotos ? `${1 + (hero?.gallery?.length || 0)} photo(s) on file` : 'None found',
