...
/**
 * Renders one thin/JS-shell page with a real browser. Takes and returns the
 * caller's own browser instance (lazily created on first use) rather than a
 * module-level shared one — this runs under concurrency (multiple
 * contractors audited in parallel, see scripts/run-audits.js), and a
 * shared variable meant one worker could close another's still-in-use
 * browser, or two workers racing `!sharedBrowser` could each launch their
 * own instance while only one got stored — orphaning the other as a
 * Chromium process nothing ever closes. That's a real leak observed in
 * production (steady RSS growth to an eventual heap-limit crash).
 */
async function renderWithBrowser(url, browserRef) {
  try {
    if (!browserRef.instance) browserRef.instance = await chromium.launch();
...
export async function crawlSiteStructure(baseUrl, { knownPhones = [], maxPages = MAX_PAGES } = {}) {
  ...
  const browserRef = { instance: null }; // this call's own browser, never shared across concurrent audits
  let host;
  try {
  ...
  while (queue.length && pages.length < maxPages) {
    ...
    if (parsed.wordCount * 6 < THIN_PAGE_CHAR_THRESHOLD && !parsed.hasForm && !parsed.hasTelLink) {
      const renderedHtml = await renderWithBrowser(finalUrl || url, browserRef);
      if (renderedHtml) {
  ...
  if (browserRef.instance) {
    await browserRef.instance.close().catch(() => {});
    browserRef.instance = null;
  }
