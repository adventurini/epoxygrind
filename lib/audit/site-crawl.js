...
export async function crawlSite(url, { timeoutMs = 30_000, knownPhones = [] } = {}) {
  ...
  try {
    ...
    // in"/certification/partner badge logos near the top, the image-quality
    // audit was frequently scoring a page's worth of logos and calling it
    // "photos," never reaching the actual job-site photography further down
    // the page (confirmed via a real audit report showing 6/6 sampled
    // images as different company wordmark logos, all scored ~40/100).
    // Filter for likely logos/badges/icons and prefer real project photos;
    // keep at most one logo for context if photos are scarce, matching a
    // human reviewer's judgment call rather than a hard exclusion.
    const images = await page.evaluate(() => {
      function looksLikeLogoOrBadge(img) {
        const haystack = `${img.src} ${img.alt} ${img.className}`.toLowerCase();
        if (/\blogo\b|\bbadge\b|\bicon\b|\bavatar\b|\bsprite\b|\bfavicon\b|\bwordmark\b|certified|accredited|\baward\b|\bbbb\b|angie|houzz|trustpilot|google-partner|yelp/i.test(haystack)) {
          return true;
        }
        if (img.closest('header, nav, footer, [class*="badge" i], [class*="certif" i], [class*="partner" i], [class*="trust" i]')) {
          return true;
        }
        // Logos/badges/icons are typically small and roughly square (a
        // wordmark can be wide, but a plain <img> logo asset is usually
        // compact); real job-site photography is normally much larger and
        // reads as a natural landscape/portrait photo aspect ratio.
        const ar = img.naturalWidth / img.naturalHeight;
        if (img.naturalWidth < 400 && ar > 0.7 && ar < 1.4) return true;
        return false;
      }
      ...
      const selected = likelyPhotos.length >= 6
