...
function contractorWriteupHtml(c) {
  ...
  // Each item is a complete, self-contained predicate (own verb), not a
  // fragment assuming a hardcoded "is" prefix — mixing adjective fragments
  // ("licensed") with verb fragments ("offers free estimates") after one
  // shared "is" produced "is offers free estimates" for ~47% of profiles.
  const credBits = [];
  if (trust.licensed && trust.insured) credBits.push('is licensed and insured');
  else if (trust.licensed) credBits.push('is licensed');
  else if (trust.insured) credBits.push('is insured');
  if (trust.free_estimates) credBits.push('offers free estimates');
  if (trust.financing) credBits.push('offers financing');
  if (trust.warranty) credBits.push('backs their work with a warranty');
  if (credBits.length) sentences.push(`${escapeHtml(c.name)} ${joinNaturally(credBits)}.`);
