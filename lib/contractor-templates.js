...
import { getContractorHero, getContractorLogo } from './contractor-images.js';
...
function firstHeroImage(contractors) {
  for (const c of contractors) {
    const hero = getContractorHero(c.state_slug, c.slug);
...
function contractorCardHtml(c) {
  ...
  const classes = ['category-card', hero && 'has-thumb', !hero && logo && 'has-logo'].filter(Boolean).join(' ');
  const thumbHtml = hero
    ? `<div class="category-card-thumb-wrap"><img class="category-card-thumb" src="${escapeHtml(hero.path)}" alt="" loading="lazy">${logo ? `<img class="category-card-logo-badge" src="${escapeHtml(logo.path)}" alt="" loading="lazy">` : ''}</div>`
    : logo
...
export function renderContractorProfile(c) {
  ...
  const logo = getContractorLogo(c.state_slug, c.slug);
  const heroHtml = hero
    ? `<figure class="contractor-hero"><img src="${escapeHtml(hero.path)}" alt="${escapeHtml(c.name)}" loading="eager">${hero.attributions?.[0] ? `<figcaption>Photo: ${escapeHtml(hero.attributions[0].display_name)} via Google</figcaption>` : ''}</figure>`
    : '';

  const galleryHtml = hero?.gallery?.length
    ? `<h2>Photos</h2><div class="contractor-gallery">${hero.gallery.map((g) =>
        `<a href="${escapeHtml(g.path)}" target="_blank" rel="noopener"><img src="${escapeHtml(g.path)}" alt="${escapeHtml(c.name)} — customer photo" loading="lazy"></a>`,
      ).join('')}</div><p class="tiny muted">Photos from Google.</p>`
    : '';
  ...
  const bodyHtml = `${breadcrumbsHtml(breadcrumbs)}
    ...
    <div class="claim-banner">
      <span>Own this business? This listing was built from public info on your website.</span>
      <a href="mailto:claims@epoxygrind.com?subject=${encodeURIComponent(`Claim listing: ${c.name}`)}">Claim this page →</a>
    </div>`;
  ...
  return renderContentPage({
    ...
    // ~6% of contractors have no Google Places photo on file — fall back to
    // their logo, then the generic shot, rather than shipping no image at all.
    ogImage: hero?.path || logo?.path || GENERIC_OG_IMAGE,
  });
}
