...
function ratingBadgeHtml(c) {
  ...
  return `<span class="rating-badge"><svg viewBox="0 0 24 24" aria-hidden="true" class="rating-google-g"><path d="M22.5 12.25c0-.76-.07-1.5-.2-2.2H12v4.16h5.9a5.05 5.05 0 0 1-2.19 3.32v2.7h3.53c2.07-1.9 3.26-4.7 3.26-7.98Z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.53-2.7c-.98.66-2.24 1.04-3.75 1.04-2.88 0-5.32-1.94-6.19-4.56H2.17v2.79A11 11 0 0 0 12 23Z" fill="#34A853"/><path d="M5.81 14.12a6.6 6.6 0 0 1 0-4.24V7.09H2.17a11 11 0 0 0 0 9.82l3.64-2.79Z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.96 1 12 1A11 11 0 0 0 2.17 7.09l3.64 2.79C6.68 7.26 9.12 5.38 12 5.38Z" fill="#EA4335"/></svg>${escapeHtml(c.google_rating)} <span class="rating-count">(${escapeHtml(c.google_review_count)})</span></span>`;
}

function contractorCardHtml(c) {
  const logo = getContractorLogo(c.state_slug, c.slug);
  return `<a class="category-card${logo ? ' has-logo' : ''}" href="/contractors/${c.state_slug}/${c.slug}/">
    ${logo ? `<img class="category-card-logo" src="${escapeHtml(logo.path)}" alt="" loading="lazy">` : ''}
