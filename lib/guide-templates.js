...
export function renderGuidePage(data) {
  ...
  // One clickable video-thumbnail-as-hero, not a redundant static "Still
  // from..." figure plus a separate embed further down — getGuideImages()
  // is derived from the exact same video, so showing both duplicated it.
...
export function renderGuidePillar(data) {
  ...
  // from..." figure plus a separate embed further down — getGuideImages()
  // is derived from the exact same video, so showing both duplicated it.
  const primaryVideoHtml = primaryVideo ? videoEmbedHtml(primaryVideo) : '';
  const images = getGuideImages(data.slug);
