...
function areasServedHtml(metro) {
  ...
  return `<h2>Areas served</h2>
    <div class="content-prose"><p>Serving the ${escapeHtml(metro.city)} metro, including ${escapeHtml(list)}${suburbs.length > 24 ? ', and surrounding areas' : ''}.</p></div>`;
}
...
export function renderCityHub(metro) {
  ...
  const nearbyHtml = nearby.length
    ? `<h2>Nearby metros</h2><div class="category-grid">${nearby.map((n) => {
        // Only Tier 1 metros have their own city page today — a Tier 2/3
        // neighbor links to its state rollup instead, since that page
        // always exists (and lists it), rather than 404ing.
        const href = n.tier === 1 ? `/epoxy-flooring/${n.state_slug}/${n.slug}/` : `/epoxy-flooring/${n.state_slug}/`;
        const label = n.tier === 1 ? 'Epoxy flooring cost & instant estimate' : `See ${escapeHtml(n.state)} pricing`;
        return `<a class="category-card" href="${href}"><h3>${escapeHtml(n.city)}, ${escapeHtml(n.state)}</h3><p>${label}</p></a>`;
      }).join('')}</div>`
    : '';
...
export function renderStateRollup(stateSlug) {
  ...
  const linkedMetros = metros.filter((m) => m.tier === 1);
  const unlinkedMetros = metros.filter((m) => m.tier !== 1);

  const tableHtml = linkedMetros.length
    ? `<div class="comparison-table-wrap"><table class="comparison-table">
        <thead><tr><th>City</th><th>2-Car Garage (Flake Epoxy)</th></tr></thead>
        <tbody>${linkedMetros.map((m) => {
  ...
  const otherCitiesHtml = unlinkedMetros.length
    ? `<h2>Other ${escapeHtml(stateName)} cities</h2><div class="content-prose"><p>We also cover pricing data for ${escapeHtml(unlinkedMetros.map((m) => m.city).join(', '))} — instant estimates are available for any of these areas even without a dedicated page yet.</p></div>`
    : '';
