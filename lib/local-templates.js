...
function estimatorCta(zip, label = 'Get my instant estimate →') {
  const href = zip ? `/estimator/?zip=${encodeURIComponent(zip)}` : '/estimator/';
  ...
  if (!local.length) {
    return stateCount
      ? `<h2>Find a contractor in ${escapeHtml(metro.city)}</h2>
        <div class="content-prose"><p>We don't have a contractor listed in ${escapeHtml(metro.city)}'s coverage area yet — <a href="${stateHref}">browse ${escapeHtml(stateCount)} contractor${stateCount === 1 ? '' : 's'} in ${escapeHtml(metro.state)}</a>, or get an instant estimate first so you know what a fair quote looks like.</p></div>`
      : '';
  }
...
export function renderCityHub(metro) {
  ...
  return renderContentPage({
    ...
    extraScripts: '<script src="/js/calculator.bundle.js" defer></script>',
...
export function renderStateRollup(stateSlug) {
  ...
  const contractorCalloutHtml = `<h2>Find a contractor in ${escapeHtml(stateName)}</h2>
    <div class="content-prose"><p>${contractorCount
      ? `Browse <a href="/contractors/${stateSlug}/">${escapeHtml(contractorCount)} listed epoxy and concrete coating contractor${contractorCount === 1 ? '' : 's'} in ${escapeHtml(stateName)}</a> — real phone numbers and service areas, no lead-buying middleman.`
