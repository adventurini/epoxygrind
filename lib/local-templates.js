...
import { contractorsByCity, contractorsByState } from './contractors.js';
    ...
    <div class="calc-shell">
      <div class="calc-panel">
        ...
        <div class="color-block">
          ...
          <label class="fld" id="flakeWrap"><span>Flake color</span>
            <div class="color-line">
              <div class="color-picker-wrap">
...
function localContractorsHtml(metro) {
  const local = contractorsByCity(metro.state_slug, metro.city);
  ...
  if (!local.length) {
    return stateCount
      ? `<h2>Find a contractor in ${escapeHtml(metro.city)}</h2>
        <div class="content-prose"><p>We don't have a vetted contractor listed in ${escapeHtml(metro.city)} yet — <a href="${stateHref}">browse ${escapeHtml(stateCount)} contractor${stateCount === 1 ? '' : 's'} in ${escapeHtml(metro.state)}</a>, or get an instant estimate first so you know what a fair quote looks like.</p></div>`
      : '';
  }
  ...
  return `<h2>Contractors in ${escapeHtml(metro.city)}</h2>
    <div class="category-grid">${cards}</div>
    <p><a href="${stateHref}">See all ${escapeHtml(stateCount)} contractor${stateCount === 1 ? '' : 's'} in ${escapeHtml(metro.state)} →</a></p>`;
}
