 ...
 * rows where the scraper had nothing to work with. Filtered here (not
 * deleted from enriched.json) so the raw scrape data stays intact.
 */
function passesQualityBar(c) {
  return Boolean(c.phones?.length) && Boolean(c.service_areas?.length);
}
