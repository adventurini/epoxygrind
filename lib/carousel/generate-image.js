...
const CHARACTER_BLOCK = 'Same character, identical face, hair, glasses, and outfit as the reference photo.';
...
const FRAMING_BLOCK = 'Full body shot, small within the frame — he occupies roughly the bottom half of a tall vertical image, with the upper third to upper half left open/empty (plain background, no props) so text can go there.';

const BUCKET = 'content-images';

export function buildImagePrompt(scene) {
  return `${CHARACTER_BLOCK} ${FRAMING_BLOCK} ${scene} ${STYLE_BLOCK} No text, no lettering.`;
}
 ...
 *
 * @param {{ masterUrl: string, prompt: string, dayId: string, position: number, generationId: string }} opts
 * @returns {Promise<{ imageUrl: string }>}
 */
export async function generateSlideImage({ masterUrl, prompt, dayId, position, generationId }) {
