...
export async function generateSlideScenes({ audience, topic }) {
  ...
  // Slide 6 always features BOTH Grinder Dad AND "the Pro" (a separate,
  // confident professional character) together — Anthony: "the captions...
  // next time, maybe hire a contractor. And a picture with the consumer
  // and the contractor together" (consumer), and "the pro contractor with
  // the dad guy at the end with the contractor" (contractor). Same two
  // characters both times, different narrative meaning per audience.
  const closerNote = audience === 'consumer'
    ...
    : '6. Closer slide — this one is NOT about marketing/ops anymore. It features BOTH Grinder Dad AND "the Pro" together, celebrating: Grinder Dad reframed as a delighted HOMEOWNER/CUSTOMER (relaxed, not working, no tools) standing with "the Pro" (the contractor who did the work) on the gleaming finished floor — a high five, handshake, or big grins together. The payoff of a contractor who fixed the problem this post is about and made a customer genuinely happy.';
