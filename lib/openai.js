...
export function isDemoMode() {
  return !getApiKey() && !isOpenArtConfigured();
}

/**
 * @param {string} imageDataUrl
 * @param {{ finish: string, sqFtOverride?: number | null, lengthFt?: number | null, widthFt?: number | null, designSummary?: string }} input
 */
export async function analyzeSpaceImage(imageDataUrl, input) {
  ...
  const overrideNote = input.sqFtOverride
    ? `The contractor entered ${input.sqFtOverride} sq ft manually — use that figure.`
    : input.lengthFt && input.widthFt
      ? `The contractor entered ${input.lengthFt} ft × ${input.widthFt} ft — use that for square footage.`
  ...
  const res = await fetch(`${OPENAI_BASE}/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: `You analyze garage and concrete floor photos for epoxy coating contractors. Return JSON only with keys:
...
analysisSummary (string, 2 sentences max for the homeowner).`,
        },
        {
          role: 'user',
          content: [
            {
              type: 'text',
              text: `Analyze this space for an epoxy floor quote. Selected finish preference: ${input.finish}. ${designNote} ${overrideNote}`,
            },
...
function demoAnalysis(input) {
  ...
  return {
    ...
    spaceType: 'Two-car residential garage',
    conditionNotes: 'Demo mode — add OPENAI_API_KEY for live AI analysis.',
    surfaceIssues: ['Minor tire marks', 'Typical wear near door'],
    recommendedFinish: input.finish || 'flake',
    analysisSummary:
      `Standard two-car garage with concrete ready for coating after prep.${designBit} Demo estimate — connect your API key for photo-based analysis.`,
  };
}
...
function demoPreview(angleId, baseColorHex) {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1024" height="1024" viewBox="0 0 1024 1024">
    ...
    <text x="512" y="500" text-anchor="middle" fill="#fff" font-family="Arial,sans-serif" font-size="28" font-weight="700">AI preview — demo mode</text>
    <text x="512" y="540" text-anchor="middle" fill="#C5D2E8" font-family="Arial,sans-serif" font-size="18">Add OPENAI_API_KEY on Vercel</text>
  </svg>`;
