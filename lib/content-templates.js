...
/** @param {string} href e.g. "/diy/how-to-epoxy-garage-floor/" -> "how-to-epoxy-garage-floor" */
function slugFromDiyHref(href) {
  const match = href.match(/^\/diy\/([^/]+)\/?$/);
  return match ? match[1] : null;
}

function diyCardHtml(item) {
  const slug = slugFromDiyHref(item.href);
  const images = slug ? getGuideImages(slug) : null;
  const thumb = images ? `<img class="category-card-thumb" src="${images.thumb}" alt="" loading="lazy">` : '';
...
export function renderDiyHubPage(data) {
  ...
  const groupHtml = (group) => `<h2>${escapeHtml(group.title)}</h2>
    <div class="category-grid">
      ${group.items.map(diyCardHtml).join('')}
    </div>`;
