...
function sectionAndSlugFromHref(href) {
  const match = href.match(/^\/(diy|compare|reviews)\/([^/]+)\/?$/);
  return match ? { section: match[1], slug: match[2] } : null;
}

/** Thumbnail source differs per section: /diy/ and /compare/ are video-backed
 * how-to/comparison content (GUIDE_VIDEOS), /reviews/ is a product deep-dive
 * so it uses the same registry photo shown on its own page (product-registry.js). */
function diyHubCardThumb(href) {
...
function diyCardHtml(item) {
  const thumbSrc = diyHubCardThumb(item.href);
