...
export function renderRankingPage(data) {
  ...
  const picksHtml = data.picks
    .map((pick) => {
      ...
      if (!resolved.ok) {
        console.error(`renderRankingPage(${data.slug}): ${pick.productId} unresolved (${resolved.reason})`);
        return null;
      }
      return `<h3>${escapeHtml(resolved.product.display_name)}</h3>
        ${verdictBoxHtml({ label: pick.verdictLabel, reason: pick.verdictReason })}
        <div class="content-prose">${pick.writeupHtml}</div>
        <p><a class="btn btn-p btn-sm" href="/go/${encodeURIComponent(pick.productId)}" rel="${resolved.rel}" target="_blank">Check price →</a></p>`;
    })
    .filter(Boolean)
    .join('<hr style="border:none;border-top:1px solid var(--line);margin:28px 0">');
