...
import { getShoppingListImage } from './shopping-list-images.js';
import { getLearnImage } from './learn-images.js';
...
// the homepage hero, for the rare page with no real product/video photo.
...
export function renderRankingPage(data) {
  ...
  const topPickResolved = resolveProductLink(data.picks[0]?.productId);
  ...
  return renderContentPage({
    ...
    showDisclosure: true,
    ogImage: (topPickResolved.ok ? topPickResolved.product.image_url : null) || GENERIC_OG_IMAGE,
  });
}
...
export function renderReviewPage(data) {
  ...
  const heroHtml = product.image_url
    ? `<figure class="review-hero"><div class="review-hero-media"><img src="${escapeHtml(product.image_url)}" alt="${escapeHtml(product.display_name)}" loading="eager"></div>${product.image_generated ? '<figcaption>Representative image — not an exact product photo</figcaption>' : ''}</figure>`
    : '';
  ...
  const productSchema = {
    ...
    // Only a verified real photo goes in structured data — an AI-generated
    // placeholder (image_generated: true) misrepresenting a specific SKU's
    // appearance to search engines/rich results would be worse than omitting it.
    ...(product.image_url && !product.image_generated ? { image: product.image_url } : {}),
    review: {
      '@type': 'Review',
  ...
  return renderContentPage({
    ...
    showDisclosure: true,
    ogImage: product.image_url || GENERIC_OG_IMAGE,
  });
}
...
export function renderConceptComparePage(data) {
  ...
  return renderContentPage({
    ...
    ogImage: getGuideImages(data.slug)?.hero || GENERIC_OG_IMAGE,
  });
}
...
export function renderDiyGuidePage(data) {
  ...
  // The hero image and the video embed used to be two separate renders of
  // the exact same YouTube thumbnail (getGuideImages() is just video-derived) —
  // one static "Still from..." figure up top, one clickable facade further
  // down. Standard practice (and what real contractor sites do) is one
  // clickable video-thumbnail-as-hero, not a still + a duplicate embed.
  ...
  return renderContentPage({
    ...
    showDisclosure: true,
    ogImage: images?.hero || GENERIC_OG_IMAGE,
  });
}
...
export function renderShoppingListPage(data) {
  ...
  // Not a single product, so there's no merchant photo — this is always
  // the AI-generated flat-lay (see shopping-list-images.js/manifest).
  const image = getShoppingListImage(data.slug);
  const heroHtml = image
    ? `<figure class="guide-hero"><img src="${escapeHtml(image.path)}" alt="${escapeHtml(data.title)}" loading="eager">${image.generated ? '<figcaption>Representative image — not an exact product photo</figcaption>' : ''}</figure>`
    : '';
  ...
  return renderContentPage({
    ...
    showDisclosure: true,
    ogImage: image?.path || GENERIC_OG_IMAGE,
  });
}
...
export function renderCompareHubPage(data) {
  ...
  // Hub-level OG image: no single item to feature, so use the first
  // listed comparison's video still as a representative shot for the section.
  const firstSlug = data.items[0]?.href.match(/^\/compare\/([^/]+)\/?$/)?.[1];
  ...
  return renderContentPage({
    ...
    wide: true,
    ogImage: (firstSlug ? getGuideImages(firstSlug)?.hero : null) || GENERIC_OG_IMAGE,
  });
}
...
function sectionAndSlugFromHref(href) {
  const match = href.match(/^\/(diy|compare|reviews|best|guides)\/([^/]+)\/?$/);
...
async function diyHubCardThumb(href) {
  ...
  // Shopping lists aren't a how-to guide (no GUIDE_VIDEOS entry) or a single
  // product — they get their own AI-generated flat-lay manifest instead.
  if (parsed.slug.startsWith('shopping-list-')) {
    return getShoppingListImage(parsed.slug)?.path || null;
  }
...
export async function renderDiyHubPage(data) {
  ...
  // Hub-level OG image: the whole section covers a lot of ground, so lead
  // with the first "Buying guides" item's real product photo as the
  // representative shot rather than picking one arbitrarily.
  const firstItem = data.groups[0]?.items[0];
  const ogImage = firstItem ? await diyHubCardThumb(firstItem.href) : null;
  ...
  return renderContentPage({
    ...
    wide: true,
    ogImage: ogImage || GENERIC_OG_IMAGE,
  });
}
...
export function renderCoverageCalculatorPage(data) {
  ...
  return renderContentPage({
    ...
    showDisclosure: true,
    // No single product/video backs a calculator tool — reuse the flagship
    // how-to guide's hero as a fitting, on-topic representative image.
    ogImage: getGuideImages('how-to-epoxy-garage-floor')?.hero || GENERIC_OG_IMAGE,
  });
}

/**
 * Type 9 — Contractor learning center `/learn/{slug}` — one article per
 * audit-engine check (lib/audit/scoring-*.js `label` fields), explaining
 * why the metric matters for leads, with real supporting stats and a real
 * verified YouTube video (never fabricated — see lib/audit/learn-links.js
 * for the check-label -> slug map the audit dashboard links through).
 */
export function renderLearnPage(data) {
  const breadcrumbs = [
    { label: 'Home', href: '/' },
  ...
  const image = getLearnImage(data.slug);
  const heroHtml = image
    ? `<figure class="guide-hero"><img src="${image.path}" alt="${escapeHtml(data.title)}" loading="eager"></figure>`
    : '';

  const statsHtml = data.stats?.length
    ? `<div class="stat-evidence">
        ${data.stats.map((s) => `<div class="stat-evidence-item"><strong>${escapeHtml(s.stat)}</strong><span>${escapeHtml(s.context)}</span><a href="${escapeHtml(s.url)}" target="_blank" rel="noopener">${escapeHtml(s.source)}</a></div>`).join('')}
      </div>`
    : '';
  ...
  const sectionsHtml = data.sections.map((s) => `<h2>${escapeHtml(s.heading)}</h2>${s.bodyHtml}`).join('');

  const relatedHtml = data.relatedLinks?.length
    ? `<h2>Related</h2><div class="related-links">
        ${data.relatedLinks.map((r) => `<a class="related-link" href="${escapeHtml(r.href)}"><strong>${escapeHtml(r.title)}</strong><span>${escapeHtml(r.description)}</span></a>`).join('')}
      </div>`
    : '';

  const { html: faqHtml, schema: faqSchema } = data.faq?.length ? faqBlock(data.faq) : { html: '', schema: null };

  const bodyHtml = `${breadcrumbsHtml(breadcrumbs)}
    ${headHtml({ eyebrow: 'Learning center', title: data.title, dek: data.dek })}
    ${heroHtml}
    <div class="content-prose">${data.introHtml}</div>
    ${statsHtml}
    ${videoHtml}
  ...
  return renderContentPage({
    ...
    bodyHtml,
    schema: faqSchema ? [articleSchema, faqSchema, breadcrumbSchema(breadcrumbs, `/learn/${data.slug}/`)] : [articleSchema, breadcrumbSchema(breadcrumbs, `/learn/${data.slug}/`)],
    ogImage: image?.path || GENERIC_OG_IMAGE,
  });
}

/** Learning-center hub `/learn/` — grouped by audit category. */
export function renderLearnHubPage(data) {
  const breadcrumbs = [{ label: 'Home', href: '/' }, { label: 'Learning center' }];

  const groupHtml = (group) => `<h2>${escapeHtml(group.title)}</h2>
    <div class="category-grid">
      ${group.items.map((item) => {
        const image = getLearnImage(item.slug);
        const thumb = image ? `<img class="category-card-thumb" src="${image.path}" alt="" loading="lazy">` : '';
        return `<a class="category-card${thumb ? ' has-thumb' : ''}" href="/learn/${item.slug}/">${thumb}<h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.description)}</p></a>`;
      }).join('')}
    </div>`;

  const bodyHtml = `${breadcrumbsHtml(breadcrumbs)}
    ${headHtml({ eyebrow: 'For contractors', title: data.title, dek: data.dek })}
    ${data.groups.map(groupHtml).join('')}
    ${contractorCtaHtml()}`;

  const firstSlug = data.groups[0]?.items[0]?.slug;
  const ogImage = firstSlug ? getLearnImage(firstSlug)?.path : null;
  ...
  return renderContentPage({
    ...
    path: '/learn/',
    bodyHtml,
    schema: [breadcrumbSchema(breadcrumbs, '/learn/')],
    wide: true,
    ogImage: ogImage || GENERIC_OG_IMAGE,
  });
}
