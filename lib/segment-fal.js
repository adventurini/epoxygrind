...
async function falSegment(imageDataUrl, point) {
  ...
  const res = await fetch(`${FAL_RUN}/${SEGMENT_MODEL}`, {
    ...
    body: JSON.stringify({
      ...
      prompt: 'floor',
      point_prompts: [{ x: point.x, y: point.y, label: 1 }],
...
export async function segmentFloor(photoDataUrl) {
  ...
  // are at the bottom)." Biased slightly above the very bottom edge (0.85,
  // not 1.0) so the point lands solidly inside the floor plane rather than
  // risking a baseboard/wall sliver in photos with little foreground floor.
  const point = { x: Math.round(photo.width / 2), y: Math.round(photo.height * 0.85) };

  let result;
  try {
    result = await falSegment(photo.dataUrl, point);
  } catch (err) {
    return {
      photo: photo.dataUrl,
      mask: null,
